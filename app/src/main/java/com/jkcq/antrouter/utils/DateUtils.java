...
// }
    ...
    public static String getResultTimeStr(long time) {
        time = time / 1000;
        long second = time % 60;
        long minute = (time % 3600) / 60;
        long hour = time / 3600;
    ...
    public static void formatText(FormatType formatType, Context context, TextView tv, int color, int strRes, String... args) {
        switch (formatType) {
            case Pointer: {
                String str = context.getString(strRes, args[0]);
			...
			String minute = String.valueOf(mins % 60);*/
                String str = context.getString(strRes, day, hours);
                SpannableString span = new SpannableString(str);
			/*span.setSpan(new RelativeSizeSpan(0.7f), day.length(), str.lastIndexOf(hours),
					Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
			span.setSpan(new RelativeSizeSpan(0.7f), str.lastIndexOf(hours) + hours.length(), str.length(),
					Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);*/
                tv.setText(span);
                tv.setTextColor(color);
                break;
            ...
            case Alone: {
                String temp = args[0];
                String str = context.getString(strRes, temp);
                SpannableString span = new SpannableString(str);
                span.setSpan(new RelativeSizeSpan(0.6f), str.indexOf(temp) + temp.length(), str.length(),
                        Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
     ...
     * 计算运动时长
     */
    public static int calSportTime(long startTime, long endTime, long pauseTime) {

        long mins = endTime - startTime - pauseTime;
        long sencend = mins / 1000;
        int min = (int) (sencend / 60);
        if (sencend % 60 == 0) {
            return min;
        } else {
            return min + 1;
