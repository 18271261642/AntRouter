...
import com.jkcq.antrouter.okhttp.callback.ResultCallback;
import com.jkcq.antrouter.okhttp.tool.ExecutorDelivery;
import com.jkcq.antrouter.okhttp.tool.ResponseDelivery;
...
//            mOkHttpClient.setRetryOnConnectionFailure(true);
    ...
    public void execute(final Request request, ResultCallback callback) {
        ...
        if (!hasNetwork(AntRouterApplication.getApp())) {
...
//        });
    ...
    public void sendFailResultCallback(final Request request, final Exception e, final ResultCallback callback) {
        ...
        Logger.e("请求失败：url = " + (null == request ? "" : request.toString()) + " ,error = " + (TextUtils.isEmpty(result) ? "" : result));
    ...
    public void cancelTag(Object tag) {
        try {
            mOkHttpClient.interceptors().remove(tag);
